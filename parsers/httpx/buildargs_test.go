...
func TestBuildArgs_OptionCannotIntroduceAFlag(t *testing.T) {
	for _, value := range []string{"-oN", "--script", "/api\n/admin", "/api\r/admin"} {
		req := registry.ExecuteRequest{
			Target:  "https://example.com",
			Options: map[string]string{"paths": value},
		}
		if args, err := buildArgs(req); err == nil {
			t.Errorf("buildArgs accepted paths=%q, produced %v", value, args)
		}
	}
}
...
func TestBuildArgs_AcceptsValidOption(t *testing.T) {
	req := registry.ExecuteRequest{
		Target:  "https://example.com",
		Options: map[string]string{"paths": "/api"},
	}
	if _, err := buildArgs(req); err != nil {
		t.Fatalf("buildArgs rejected a valid paths option: %v", err)
	}
}
