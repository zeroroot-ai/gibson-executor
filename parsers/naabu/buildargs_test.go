...
func TestBuildArgs_TargetRejections(t *testing.T) {
	for _, target := range []string{
		...
		"10.0.0.1\x00",               // NUL byte
		"10.0.0.1 evil.example.com",  // space splits the token
		"10.0.0.1;id",                // shell metacharacter
		"",                           // absent
	} {
...
func TestBuildArgs_AcceptsValidTarget(t *testing.T) {
	args, err := buildArgs(registry.ExecuteRequest{Target: "10.0.0.1"})
	if err != nil {
