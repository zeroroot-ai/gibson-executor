...
# check for this repo.
#
# Why an aggregator at all (#341, #369)
# ------------------------------------
# The merge queue's required_status_checks used to name exactly one job out of
# the eight that run. `go-ci`, `Lint (golangci-lint)`, `binary-smoke-test`,
# `no-legacy-auth-guard` and CodeQL all ran, could all report red, and none of
# them could stop a merge. #369 demonstrated it: PR #365 merged with lint red,
# and the findings were fixed after the fact in #367.
#
...
#     runs. That is the "guard that cannot fail" class this repo has now been
#     bitten by three times (#336, #341, #369). The results must be read.
#
#   * `skipped` counts as PASS. That is deliberate and load-bearing: gates are
#     lane-split (`fast` on pull_request, `heavy` on merge_group) and
#     path-gated, so on any given run most gates legitimately do not execute.
